...
class File(models.Model):
    ...
    def _compute_extension(self):
        _logger.info("def _compute_extension(self):")
        for record in self:
            _logger.info(record.name)
            _logger.info(record.res_mimetype)
            _logger.info(record.content)
            _logger.info(file.guess_extension(record.name, record.res_mimetype, record.content))
    ...
    def create(self, vals_list):
        new_vals_list = []
        _logger.info("@api.model_create_multi")
