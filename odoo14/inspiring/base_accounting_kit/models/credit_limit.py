...
class ResPartner(models.Model):
    ...
    warning_stage = fields.Float(string='Warning Amount',
                                 help="A warning message will appear once the "
                                      ...
                                      " disable this feature", track_visibility=True)
    blocking_stage = fields.Float(string='Blocking Amount',
                                  help="Cannot make sales once the selected "
                                       ...
                                       "this feature", track_visibility=True)
    due_amount = fields.Float(string="Total Sale",
                              compute="compute_due_amount")
    active_limit = fields.Boolean("Active Credit Limit", default=False, track_visibility=True)
...
class SaleOrder(models.Model):
    ...
    p_has_due = fields.Boolean()
    p_is_warning = fields.Boolean()
    p_due_amount = fields.Float(related='partner_id.parent_id.due_amount')
    ...
    def _action_confirm(self):
        ...
        if self.partner_id.active_limit \
            ...
            if self.due_amount >= self.partner_id.blocking_stage:
                if self.partner_id.blocking_stage != 0 and self.env.is_admin() != True:
                    raise UserError(_(
        ...
        if self.partner_id.parent_id:
            if self.partner_id.parent_id.active_limit \
                    and self.partner_id.parent_id.enable_credit_limit:
                if self.p_due_amount >= self.partner_id.parent_id.blocking_stage:
                    if self.partner_id.parent_id.blocking_stage != 0 and self.env.is_admin() != True:
                        raise UserError(_(
                            "%s is in  Blocking Stage and "
                            "has a due amount of %s %s to pay") % (
                                            self.partner_id.parent_id.name, self.p_due_amount,
                                            self.currency_id.symbol))
    ...
    def check_due(self):
        ...
        if self.partner_id.parent_id:
            if self.partner_id.parent_id.due_amount > 0 \
                    and self.partner_id.parent_id.active_limit \
                    and self.partner_id.parent_id.enable_credit_limit:
                self.p_has_due = True
            else:
                self.p_has_due = False
        else:
            self.p_has_due = False
        ...
        if self.partner_id.parent_id:
            if self.partner_id.parent_id.active_limit\
                    and self.partner_id.parent_id.enable_credit_limit:
                if self.p_due_amount >= self.partner_id.parent_id.warning_stage:
                    if self.partner_id.parent_id.warning_stage != 0:
                        self.p_is_warning = True
                        self.p_due_amount = self.partner_id.parent_id.due_amount
            else:
                self.p_is_warning = False
        else:
            self.p_is_warning = False
        ...
        for rec in self:
            if rec.partner_id.active_limit and rec.move_type in pay_type \
                ...
                if rec.due_amount >= rec.partner_id.blocking_stage:
                    if rec.partner_id.blocking_stage != 0 and self.env.is_admin() != True:
                        raise UserError(_(
                            "%s is in  Blocking Stage and "
                            "has a due amount of %s %s to pay") % (
            ...
            if rec.partner_id.parent_id:
                if rec.partner_id.parent_id.active_limit \
                        and rec.partner_id.parent_id.enable_credit_limit:
                    if rec.p_due_amount >= rec.partner_id.parent_id.blocking_stage:
                        if rec.partner_id.parent_id.blocking_stage != 0 and self.env.is_admin() != True:
                            raise UserError(_(
                                "%s is in  Blocking Stage and "
                                "has a due amount of %s %s to pay") % (
                                                rec.partner_id.parent_id.name, rec.p_due_amount,
                                                rec.currency_id.symbol))
