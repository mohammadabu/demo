odoo.define('aspl_product_expiry_alert.dashboard', function (require) {
    ...
    var ProductExpiryDashboard = AbstractAction.extend({
        ...
        open_expire: function(){
            var self = this;
            var yesterday_date = moment().subtract(1, 'day')
            var expiration_from = yesterday_date.locale('en').format("DD/MM/YYYY 00:00:00");
            var expiration_to = yesterday_date.locale('en').format("DD/MM/YYYY 23:59:59");
            self.do_action({
                   ...
                   target: 'new',
                   domain:[['state_check','=','expired'], ['expiration_date', '>=', expiration_from], ['expiration_date', '<=', expiration_to]],
            });
        },
        start: function () {
            this._super.apply(this, arguments);
