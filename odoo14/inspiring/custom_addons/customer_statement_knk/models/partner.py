...
class ResPartner(models.Model):
    ...
    def get_customer_opbalance(self, from_date, to_date, account_type):
        domain = [('move_id.partner_id', '=', self.id), ('move_id.state', '=', 'posted')]
        if account_type == 'receivable':
            domain += [('account_id.internal_type', '=', 'receivable')]
        elif account_type == 'payable':
            domain += [('account_id.internal_type', '=', 'payable')]
        else:
            domain += [('account_id.internal_type', 'in', ['payable', 'receivable'])]
        debit = 0.0
        credit = 0.0
        balance = 0.0
        if from_date:
    ...
    def get_customer_statements(self, from_date, to_date, account_type, target_moves='all_entries'):
    ...
    def get_agedheader(self, from_date, to_date, aged_type):
        res = {}
        period_length = 30
        if not from_date:
    ...
    def get_ageddata(self, from_date, to_date, account_type, aged_type):
        target_move = 'posted'
        types = [] 
        if account_type == 'receivable':
            types = ['receivable']
        ...
        if not from_date:
            from_date = fields.Date.today()
        movelines, total, total = self._get_partner_move_lines(
            types, from_date, target_move,30)
        return movelines
