...
class InsPartnerLedger(models.TransientModel):
    ...
    def build_detailed_move_lines(self, offset=0, partner=0, fetch_range=FETCH_RANGE):
        ...
        if data.get('initial_balance'):
            ...
            _logger.info('initial_balance_opening_balance %s', opening_balance)
        sql = ('''
                    SELECT 
                        COALESCE(SUM(l.debit - l.credit),0) AS balance
                    FROM account_move_line l
        ...
        for running_balance in running_balance_list:
            ...
            _logger.info('running_balance_opening_balance %s', opening_balance)
        ...
        sql = ('''
                SELECT
                    ...
                    m.invoice_partner_display_name AS invoice_partner_display_name,
        ...
        cr.execute(sql)
        _logger.info("----------------cr.execute---------------------")
        for row in cr.dictfetchall():
            if data.get('childs_ids'):
                if row['m_partner_id'] not in data.get('childs_ids'):
                    # to display only filtered childs 
                    continue

            current_balance = row['balance']
            row['balance'] = opening_balance + current_balance
            opening_balance += current_balance
            _logger.info('current_balance_opening_balance %s', opening_balance)

            row['initial_bal'] = False
            move_lines.append(row)
            partner = self.env['res.partner'].browse(row['m_partner_id'])
            if partner.display_name:
                row['partner_display_name'] = partner.display_name
            else:
                row['partner_display_name'] = row['invoice_partner_display_name']

        if ((count - offset_count) <= fetch_range) and data.get('initial_balance'):
            # NOTE: This is where the total is getting calculated, need the partner to be added instead of "l"
            if data.get('childs_ids_pdf'):
                # to display only filtered childs 
                WHERE_FULL += " AND m.partner_id IN (%s)" % ''.join(str(n) for n in data.get('childs_ids_pdf'))
            ...
            for row in cr.dictfetchall():
                ...
                row['company_currency_id'] = currency_id.id
                move_lines.append(row)
                _logger.info("========FOR ROW========")
                _logger.info(row)
            # move_lines.pop()
            # last_row = {}
            # last_row['balance'] = '21312312'
            # last_row['credit'] = "21321312"
            # last_row['debit'] = "213213321"
            # last_row['move_name'] = "Ending Balance"
            # last_row['partner_id'] = "ALMosa"
            # last_row['company_currency_id'] = 135
            # last_row['ldate'] = ""
            # move_lines.append(last_row)
    ...
    def process_data(self):
        ...
        for partner in partner_ids:
            ...
            for row in current_lines:
                if data.get('childs_ids_pdf'):
                    if row['m_partner_id'] not in data.get('childs_ids_pdf'):
                        # to display only filtered childs 
                        continue

                row['partner_display_name'] = row['invoice_partner_display_name'] or ''
                row['initial_bal'] = False
                row['ending_bal'] = False
            ...
            # WHERE_FULL += " AND p.name LIKE '%s'" % partner.name

            ## This changes the colapsed menu values
            # WHERE_FULL += " AND p.id = 9"
            # WHERE_FULL += " AND l.partner_id = 306"
            sql = ('''
                SELECT 
                    COALESCE(SUM(l.debit),0) AS debit, 
                    COALESCE(SUM(l.credit),0) AS credit, 
            ...
            cr.execute(sql)
            for row in cr.dictfetchall():
                _logger.info('row_balance %s', row['balance'])
    ...
    def get_filters(self, default_filters={}):
        ...
        childs_ids = []
        if self.partner_ids:
            for p_ch in self.partner_ids:
                partner = self.env['res.partner'].browse(p_ch.id)
                childs_ids_pdf.append(partner.id)
                childs_ids.append(partner.id)
                if partner.child_ids:
                    for p_child in partner.child_ids:
                        childs_ids.append(p_child.id)
                        childs_ids_pdf.append(p_child.id)

                if partner.parent_id.id:
                    partners_ids.append(partner.parent_id.id)
                else:
        ...
        filter_dict = {
            ...
            'partner_ids': list(set(partners_ids)),
            'childs_ids_pdf': childs_ids_pdf,
    ...
    def action_pdf_client(self):
        filters, account_lines = self.get_report_datas()
        return self.env.ref(
            'account_dynamic_reports'
            '.action_print_partner_ledger_client').with_context(landscape=True).report_action(
    ...
    def action_xlsx(self):
            ...
            for line in account_lines:
                ...
                if filter.get('include_details', False):
                    ...
                    count, offset, sub_lines = record.build_detailed_move_lines(offset=0, partner=line,
                                                                                     fetch_range=1000000)
                    _logger.info("if filter.get('include_details', False):")
                    _logger.info(count)
                    _logger.info(offset)
                    _logger.info(sub_lines)
        ...
        return {
            ...
            'url': '/web/binary/download_document?model=common.xlsx.out&field=filedata&id=%s&filename=%s.xlsx' % (
                report_id.id, 'Partner Ledger.xlsx'),
            'target': 'new',
        }
