odoo.define('account_dynamic_reports.DynamicTbMain', function (require) {
        ...
        plot_data: function (initial_render = true) {
            ...
            self._rpc({
                ...
                if (initial_render) {
                    ...
                    self.$el.find('#date_from').datepicker({ dateFormat: 'dd-mm-yy', changeMonth: true, changeYear: true });
                    self.$el.find('#date_to').datepicker({ dateFormat: 'dd-mm-yy', changeMonth: true, changeYear: true });
                    self.$el.find('#date_from_cmp').datepicker({ dateFormat: 'dd-mm-yy', changeMonth: true, changeYear: true });
                    self.$el.find('#date_to_cmp').datepicker({ dateFormat: 'dd-mm-yy', changeMonth: true, changeYear: true });
    ...
    var DynamicGlMain = AbstractAction.extend({
        ...
        update_with_filter: function (event) {
            event.preventDefault();
            console.log("update_with_filter")
            var self = this;
            self.initial_render = false;
        ...
        plot_data: function (initial_render = true) {
            ...
            self._rpc({
                ...
                if (initial_render) {
                    ...
                    self.$el.find('#as_on_date').datepicker({ dateFormat: 'dd-mm-yy', changeMonth: true, changeYear: true });
    ...
    var DynamicPlMain = AbstractAction.extend({
        ...
        client_pdf: function (e) {
            ...
            self._rpc({
                ...
                var action = {
                    'type': 'ir.actions.report',
                    'report_type': 'qweb-pdf',
                    'report_name': 'account_dynamic_reports.partner_ledger_client',
                    'report_file': 'account_dynamic_reports.partner_ledger_client',
                    'data': { 'js_data': data },
                    'context': {
                    ...
                    'display_name': 'Customer Partner Ledger',
                };
                return self.do_action(action);
            });
        },
        ...
        fetch_move_lines: function (event) {
            ...
            var offset = 0;
            var td = $(event.currentTarget).next('tr').find('td');
            var temp_final_debit = 0;
            var temp_final_credit = 0;
            var temp_final_balance = 0;

            console.log("SubSectionPl new 1")
            if (td.length == 1) {
                self.loader_disable_ui();
                self.pl_lines_by_page(offset, account_id).then(function (datas) {
                    var last_item = datas[2][datas[2].length -1];
                    var last_item_credit = datas[2][datas[2].length - 1]["credit"];
                    var last_item_debit = datas[2][datas[2].length -1]["debit"];
                    var last_item_balance = datas[2][datas[2].length -1]["balance"];
                    console.log(last_item_debit);
                    ...
                    _.each(datas[2], function (k, v) {
                        ...
                        console.log(k.debit);
                        temp_final_debit += k.debit;
                        temp_final_credit += k.credit;
                        temp_final_balance += k.balance;
                        k.debit = self.formatWithSign(k.debit, formatOptions, k.debit < 0 ? '-' : '');
                        k.credit = self.formatWithSign(k.credit, formatOptions, k.credit < 0 ? '-' : '');
                        k.balance = self.formatWithSign(k.balance, formatOptions, k.balance < 0 ? '-' : '');
                        k.ldate = field_utils.format.date(field_utils.parse.date(k.ldate, {}, { isUTC: true }));
                    });
                    datas[2][datas[2].length -1]["debit"] = (temp_final_debit - last_item_debit).toFixed(2) + ' SR';
                    datas[2][datas[2].length - 1]["credit"] = (temp_final_credit - last_item_credit).toFixed(2) + ' SR';
                    datas[2][datas[2].length - 1]["balance"] = (temp_final_balance - last_item_balance).toFixed(2) + ' SR';
