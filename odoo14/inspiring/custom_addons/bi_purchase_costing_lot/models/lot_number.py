...
class StockProductionLot(models.Model):
    ...
    real_cost = fields.Float(string="Purchase Costs/Uom",compute='_compute_product_real_landed_cost',digits=dp.get_precision('Lot Costing'))
    real_actual_cost = fields.Monetary(string="Actual Unit Cost")
    landed_cost_amount = fields.Float(string="Landed Costs",digits=dp.get_precision('Lot Costing'),compute="_compute_product_real_landed_cost")
    total_purchased_qty = fields.Monetary(string="Purchased Quantity",compute="_compute_total_purchase_qty")
    real_cost_available_stock = fields.Monetary(string="Available Stock Cost",compute="_compute_real_cost_available_stock")
    ...
    cost_based_avail_stock = fields.Monetary(string="Total Cost Based On Available Stock",compute="_compute_avail_stock_unit")
    ...
    def _compute_product_real_landed_cost(self):
        quant_obj = self.env['stock.quant'].sudo().search([('lot_id','=',self.id),('location_id.usage','=','internal')])
        
        stock_move_line_res = self.env['stock.move.line'].sudo().search([('lot_id','=',self.id)],limit=1)
        ...
        if stock_move_line_obj != False :
            if stock_move_line_obj.picking_id.picking_type_id.code == 'incoming' :
                ...
                landed_res = self.env['stock.landed.cost'].sudo().search([('picking_ids','in',[stock_move_line_obj.picking_id.id])])

                landed_res_lines = self.env['stock.valuation.adjustment.lines'].sudo().search([('product_id','=',self.product_id.id),
                ...
                for line in landed_res_lines :
                    if not (line.product_id.id,line.cost_line_id.id,line.quantity) in product_lst :

                        total =  total + line.additional_landed_cost
                        product_lst.append((line.product_id.id,line.cost_line_id.id,line.quantity))
            quantity = self.total_purchased_qty
            
            price = 0
            purchase_order_res = self.env['purchase.order'].search([('name','=',stock_move_line_obj.picking_id.origin)])
            ...
            for line in purchase_order_res.order_line :
                if line.product_id.id == self.product_id.id :
                    ...
                    price = purchase_currency._convert(price, purchase_company.currency_id, purchase_company, purchase_order_res.date_order.date() or fields.Date.today())
