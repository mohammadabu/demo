...
class DynamicPurchaseReport(models.Model):
    ...
    def purchase_report(self, option):
        ...
        if report_values.product_ids:
            _logger.info('report_values_products_ids %s', report_values.product_ids.ids)
            data.update({
                'products_ids': report_values.product_ids.ids,
            })
    ...
    def get_filter(self, option):
        ...
        filters['products_list'] = data.get('products_list')
        filters['products_ids'] = data.get('products_ids')
        filters['report_type_tech'] = data.get('report_type')
    ...
    def get_filter_data(self, option):
        ...
        filter_dict = {
            ...
            'products_list': [(p.id, p.name) for p in products],
        }
    ...
    def _get_report_sub_lines(self, data, report, date_from, date_to):
        ...
        if data.get('report_type') == 'report_by_order':
            ...
            _logger.info('data_products_ids %s', data.get('products_ids'))
           ...
           select l.name,l.date_order,l.partner_id,l.amount_total,l.notes,l.user_id,res_partner.name as partner,
            ...
            if data.get('date_to'):
                query += term + "l.date_order <= '%s' " % data.get('date_to')
            if data.get('products_ids'):
                query += term + "purchase_order_line.product_id IN %s " % str(tuple(data.get('products_ids')) + tuple([0]))
