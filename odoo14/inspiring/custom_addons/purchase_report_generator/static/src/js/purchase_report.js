...
//                self.apply_filter();
		...
		load_data: function(initial_render = true) {
			...
			self._rpc({
				...
				if (initial_render) {
					...
					self.$el.find('.products-multiple').select2({
		...
		apply_filter: function() {
			...
			if ($(".products-multiple").length) {
				var product_ids = [];
           		var product_list = $(".products-multiple").select2('data')
			...
			if ($(".report_type").length) {
				...
				let types = ["report_by_order_detail", "report_by_order_detail"];
				let selected_type = $(".report_type").find(":selected").val();
				if (types.includes(selected_type)) {
					$("#products_filter").css('display', 'block')
				} else {
